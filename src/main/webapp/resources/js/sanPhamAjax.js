$(document).ready(function() {
	...
	function ajaxGet(page){
		...
		$.ajax({
			...
			url: "http://localhost:8080/store/api/san-pham/all" + '?page=' + page,
			success: function(result){
				$.each(result.content, function(i, sanPham){
					var sanPhamRow = '<tr>' +
					                  '<td>' + '<img src="/store/img/'+sanPham.id+'.png" class="img-responsive" style="height: 50px; width: 50px" />'+'</td>' +
					                  ...
					                  '<td> <button class="btn btn-warning btnChiTiet" style="margin-right: 6px"> <span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span> </button>' ;
					
					var checkTenDanhMuc = (sanPham.danhMuc.tenDanhMuc.toLowerCase()).indexOf("Laptop".toLowerCase());
					sanPhamRow += ( checkTenDanhMuc != -1)?'<button class="btn btn-primary btnCapNhatLapTop" > <span class="glyphicon glyphicon-edit" aria-hidden="true"></span></button>':'<button class="btn btn-primary btnCapNhatOther" >Cập nhật</button>';
					sanPhamRow += '  <button class="btn btn-danger btnXoaSanPham"> <span class="glyphicon glyphicon-trash" aria-hidden="true"></span> </button></td>'+'</tr>';
    ...
    function ajaxPostLapTop() {
    	// PREPATEE DATA
    	 ...
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "http://localhost:8080/store/api/san-pham/save",
    ...
    $(document).on("click",".btnCapNhatLapTop", function(event){
		...
		var href = "http://localhost:8080/store/api/san-pham/"+sanPhamId;
    ...
    $(document).on("click",".btnXoaSanPham", function() {
		...
		if(confirmation){
		  $.ajax({
			  async:false,
			  type : "DELETE",
			  url : "http://localhost:8080/store/api/san-pham/delete/" + sanPhamId,
    ...
    $(document).on("click",".btnChiTiet", function() {
    	...
    	var href = "http://localhost:8080/store/api/san-pham/"+sanPhamId;
		$.get(href, function(sanPham) {
			$('.hinhAnh').attr("src", "/store/img/"+sanPham.id+".png");
