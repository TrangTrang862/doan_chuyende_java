$(document).ready(function(){
	...
	function ajaxGet(page){
		$.ajax({
			type: "GET",		
			url: "http://localhost:8080/store/api/nhan-hieu/all?page=" + page,
				...
				$.each(result.content, function(i, nhanHieu){
					var nhanHieuRow = '<tr style="text-align: center;">' +
					                  ...
					                  '<td>'+'<input type="hidden" value=' + nhanHieu.id + '>'
					                     + '<button class="btn btn-primary btnCapNhatNhanHieu" ><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></button>' +
					                     '   <button class="btn btn-danger btnXoaNhanHieu"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></button></td>'
				                      '</tr>';
	...
	function ajaxPost(){
    	...
    	$.ajax({
			...
			url : "http://localhost:8080/store/api/nhan-hieu/save",
    ...
    $(document).on("click",".btnCapNhatNhanHieu",function(){
		...
		var href = "http://localhost:8080/store/api/nhan-hieu/" + nhanHieuId;
    ...
    function ajaxPut(){
    	...
    	$.ajax({
			...
			type : "PUT",
			contentType : "application/json",
			url : "http://localhost:8080/store/api/nhan-hieu/update",
			data : JSON.stringify(formData),
            // dataType : 'json',
			success : function(response) {
    ...
    $(document).on("click",".btnXoaNhanHieu", function() {
		...
		if(confirmation){
		  $.ajax({
			  type : "DELETE",
			  url : "http://localhost:8080/store/api/nhan-hieu/delete/" + nhanHieuId,
