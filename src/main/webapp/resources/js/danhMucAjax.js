$(document).ready(function(){
	...
	function ajaxGet(page){
		$.ajax({
			type: "GET",		
			url: "http://localhost:8080/store/api/danh-muc/all" + "?page=" + page,
				...
				$.each(result.content, function(i, danhMuc){
					var danhMucRow = '<tr style="text-align: center;">' +
					                  ...
					                  '<td>'+'<input type="hidden" value=' + danhMuc.id + '>'
					                     + '<button class="btn btn-primary btnCapNhatDanhMuc" ><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></button>' +
					                     '   <button class="btn btn-danger btnXoaDanhMuc"> <span class="glyphicon glyphicon-trash" aria-hidden="true"></span> </button></td>'
				                      '</tr>';
...
//    });
	...
	function ajaxPost(){
    	...
    	$.ajax({
			...
			url : "http://localhost:8080/store/api/danh-muc/save",
    ...
    $(document).on("click",".btnCapNhatDanhMuc",function(){
		...
		var href = "http://localhost:8080/store/api/danh-muc/"+danhMucId;
    ...
    function ajaxPut(){
    	...
    	$.ajax({
			...
			type : "PUT",
			contentType : "application/json",
			url : "http://localhost:8080/store/api/danh-muc/update",
			data : JSON.stringify(formData),
            // dataType : 'json',
			success : function(response) {
    ...
    $(document).on("click",".btnXoaDanhMuc", function() {
		...
		if(confirmation){
		  $.ajax({
			  type : "DELETE",
			  url : "http://localhost:8080/store/api/danh-muc/delete/" + danhMucId,
