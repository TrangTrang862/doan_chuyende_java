$(document).ready(function() {
	...
	function ajaxGet(page){		
		...
		$.ajax({
			...
			url: "http://localhost:8080/store/api/tai-khoan/all" + '?page=' + page,
...
//					                  '<td><button class="btn btn-primary btnCapNhat" >Cập nhật</button></td>' + 
					                  '<td><button class="btn btn-danger btnXoa" ><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></button></td>';			;
    ...
    function ajaxPostTaiKhoan() {    	
    	...
    	console.log(data);
    	 ...
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "http://localhost:8080/store/api/tai-khoan/save",
    ...
    $(document).on("click",".btnXoa", function() {
		...
		if(confirmation){
		  $.ajax({
			  type : "DELETE",
			  url : "http://localhost:8080/store/api/tai-khoan/delete/" + taiKhoanId,
